...
export default function Contact() {
  ...
  const dizzyRef = useRef<gsap.core.Timeline | null>(null);
  ...
  useEffect(() => {
    const timeout = setTimeout(() => setSvgVisible(true), 1400); // 1s delay + 0.4s da transição
    ...
    const ctx = gsap.context(() => {
      ...
      function updateScreenCoords(event: MouseEvent) {
        ...
        if (!dizzyIsPlaying.current && Math.abs(event.movementX) > 500) {
          ...
          dizzyRef.current?.restart(); // agora usa ref
        }
      }
      ...
      function animateFace() {
        ...
        const yHigh =
          percentage(yPosition.current, dimensions.current.height) - 20;
        const yLow =
          percentage(yPosition.current, dimensions.current.height) - 80;
      ...
      // timeline de piscar
      blinkRef.current = gsap.timeline({
        repeat: -1,
        repeatDelay: 5,
        paused: true,
      });
      ...
      blinkRef.current
        ...
        .to(
          ".eye-right-2 , .eye-left-2",
          { duration: 0.01, opacity: 0.15 },
          0.15
        );

      // timeline de "dizzy"
      dizzyRef.current = gsap.timeline({
        paused: true,
        onComplete: () => {
      ...
      dizzyRef.current
        ...
        // exemplo de rotação para efeito "tonto"
        .to(".face", { rotate: 10, duration: 0.1, yoyo: true, repeat: 5 })
        .to(".eyes", { opacity: 1, duration: 0.01 })
        .to(".dizzy", { opacity: 0, duration: 0.01 })
        .to(".mouth", { opacity: 1, duration: 0.01 })
        .to(".oh", { opacity: 0, duration: 0.01 });
    ...
    <div className="flex flex-col items-center justify-center h-full mx-auto container xl:pt-8">
      <div className="dark:bg-[#232329] bg-[#BA9470] text-primary dark:text-white p-8 rounded-lg flex flex-col md:flex-row items-center w-full">
        <div className="md:w-1/2 mb-4 md:mb-0">
          ...
          <p className="mb-2">
            <span className="font-bold underline">Pop me an email</span> at
            hi@aa.com
          </p>
          <p className="mb-4">or give me a shout on social media.</p>
          <div className="flex space-x-4">
            <a
              href="https://github.com/lizis-bianca"
              className="dark:text-accent text-[#5e7153] text-2xl"
            >
              <FaGithub />
            </a>
            <a
              href="https://www.linkedin.com/in/lizis-bianca"
              className="dark:text-accent text-[#5e7153] text-2xl"
            >
              <FaLinkedinIn />
            </a>
        ...
        <div className="md:w-1/2 flex justify-center items-center overflow-hidden">
            <svg
              ref={svgRef}
              className="me flex-shrink-0 z-50"
              width="500"
              height="400"
              ...
              <g clipPath="url(#background-clip)">
                <g className="me" opacity="0">
                  <g className="body">
                    <path
                      className="shadow"
                      d="M139.86,51.47s12.76,8.91,16,18.07,13,39.06,17.71,49.82,15.06,32.5,15.91,36.42,3.57,9-2.52,14.2-20.63,16-20.63,16l-15.47,10.8s20.87,15.67,23.68,25.32a91.16,91.16,0,0,1,6.34,22.79l-27.39,8.54L127.2,182.85l-17-15.1L102.31,93.37Z"
                      opacity="0.09"
                      style={{ isolation: "isolate" }}
                    />
                    <path
                      className="shadow"
                      d="M70.44,74A43.64,43.64,0,0,0,70.91,84.27c-8.39,11.87-8.1,40.52-11.61,51.23S54.76,154.26,55.93,165s6.64,18.27,6.64,18.27a86.65,86.65,0,0,1,22.93,5.88c11,3.89,26.42,10.48,26.42,10.48S79.6,188.53,77.84,198a87.23,87.23,0,0,0-4.34,22.26l26.89,5.9,20.19-42,15.42-16L121.91,95Z"
                      opacity="0.09"
                      style={{ isolation: "isolate" }}
                    />
                    <path
                      className="hair-back hair"
                      d="M127.63,45.17c7,4.5,18,12,23,33,4,25,5,31.5,6,36.5s3.5,17.5,9,27c3.5,7,4.5,12,8,17s5.5,6.5,8,10c2.5,3.5,4.5,9.5,4,12l-10,10s7-20-2-26c0,0,4.5,18,3.5,21s-10,11-10,11-4.5-8.5-8-17,5,17,3,18-22,12-33,9.5-25.5-7.5-27.5-8c0,0,4-4.5,2-12.5s-4-11-3.5-9.5c0.5,1.5-2,3.5-2,10.5a9.5,9.5,0,0,1-4,8.5s-9.5-6-16-8.5A58,58,0,0,1,52.5,145s-7.5-14-2-28S58.5,106,58.5,91s3.5-35.5,24-47.5S118.5,39.5,127.63,45.17Z"
      ...
      <h1 className="dark:text-white text-primary text-4xl font-bold mt-8">
        Hey there!
      </h1>
    </div>
  );
